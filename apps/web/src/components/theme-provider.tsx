...
import { Moon, Sun, Monitor, Leaf, Eye, Zap } from "lucide-react" // Added new icons
...
export function ThemeProvider({
  ...
  // Funktion zum Anwenden des effektiven Themes
  const applyEffectiveTheme = (currentTheme: Theme) => {
    const root = window.document.documentElement
    root.classList.remove("light", "dark", "eco", "highContrast")

    let effectiveTheme: string

    switch (currentTheme) {
      case "system":
        // System-Theme: Folgt den Systemeinstellungen
        effectiveTheme = window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light"
        break
      case "auto":
        // Auto-Theme: Intelligente Anpassung basierend auf Tageszeit und System
        const hour = new Date().getHours()
        const systemDark = window.matchMedia("(prefers-color-scheme: dark)").matches
        effectiveTheme = hour > 18 || hour < 6 ? "dark" : systemDark ? "dark" : "light"
        break
      default:
        // Direkte Themes: light, dark, eco, highContrast
