...
async function rateLimitedRequest(url: string, params: any, headers: any) {
  const now = Date.now();
...
export async function searchAddress(query: string): Promise<GeocodeResult[]> {
  try {
    ...
    const nominatimResponse = await rateLimitedRequest(`${NOMINATIM_URL}/search`, {
      q: query,
      format: 'json',
...
function parseNominatimResults(data: any[]): GeocodeResult[] {
...
function parsePhotonResults(data: any): GeocodeResult[] {
  ...
  return data.features.map((feature: any) => ({
    ...
    display_name: feature.properties?.name || feature.properties?.display_name || 'Unbekannte Adresse',
